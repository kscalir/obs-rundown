...
function writeIdsToUrl({ showId, episodeId, itemId }) {
  try {
    const u = new URL(window.location.href);
    if (showId == null) u.searchParams.delete("showId"); else u.searchParams.set("showId", String(showId));
    if (episodeId == null) u.searchParams.delete("episodeId"); else u.searchParams.set("episodeId", String(episodeId));
    if (itemId == null) u.searchParams.delete("itemId"); else u.searchParams.set("itemId", String(itemId));
...
export function SelectionProvider({ children, initial }) {
  ...
  // 2) keep URL in sync when state changes
  useEffect(() => { writeIdsToUrl({ showId, episodeId, itemId }); }, [showId, episodeId, itemId]);
