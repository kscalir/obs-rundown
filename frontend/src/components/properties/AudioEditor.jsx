...
export default function AudioEditor({ selectedItem, itemData, setRefreshKey }) {
  ...
  const fetchMedia = async () => {
    try {
      const res = await fetch(`${API_BASE_URL}/api/media?showId=${selectedItem.showId}&type=audio`);
  ...
  const handleDataChange = async (field, value) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error("Failed to update audio data");
      setRefreshKey((k) => k + 1);
    } catch (err) {
      console.error("Error updating audio data:", err);
      setError("Failed to update audio data");
    }
  };
    ...
    <div style={{ padding: 15 }}>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <input 
          ...
          placeholder="Enter title for this audio item"
        />
      </div>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <label style={{ display: "flex", alignItems: "center", marginBottom: 8 }}>
          <input
            type="checkbox"
            checked={localData.fadeIn || false}
            onChange={(e) => handleDataChange('fadeIn', e.target.checked)}
            style={{ marginRight: 8 }}
          />
          Fade in
        </label>
        ...
        <label style={{ display: "flex", alignItems: "center", marginBottom: 12 }}>
          <input
            type="checkbox"
            checked={localData.fadeOut || false}
            onChange={(e) => handleDataChange('fadeOut', e.target.checked)}
            style={{ marginRight: 8 }}
          />
          Fade out
        </label>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <select
          value={localData.audioType || "music"}
          onChange={(e) => handleDataChange('audioType', e.target.value)}
          style={{
            width: "100%",
            padding: "8px",
            border: "1px solid #ddd",
